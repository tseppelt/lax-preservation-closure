...
\begin{document}
	\begin{frontmatter}
		\title{Logical~Equivalences, Homomorphism~Indistinguishability, and Forbidden~Minors} 

		\author[1]{Tim Seppelt\fnref{fn1}}
		\ead{seppelt@cs.rwth-aachen.de}
		
		\affiliation[1]{organization={RWTH Aachen University},addressline={Ahornstra{\ss}e 55}, 
			city={Aachen},
			postcode={52074}, 
...
% lax begin Lax871432.LovaszTheorem
...
% lax end
	...
	For further results in the same vein, see \cref{tab:overview}.
...
% lax begin Lax871432.MinorsComplements
...
% lax begin Lax871432.RobersonConjecture
...
% lax begin Lax871432.HomomorphismCounts
...
% lax end
% lax begin Lax871432.HomomorphismIndistinguishability
...
% lax begin Lax871432.DistinguishingClosure
	...
	A graph class $\mathcal{F}$ is \emph{homomorphism distinguishing closed} if $\cl(\mathcal{F}) = \mathcal{F}$.
% lax end
% lax begin Lax871432.DistinguishingClosureOperator
% lax begin Lax871432Proofs.isClosureOperator
	Note that $\cl$ is a closure operator in the sense that $\cl(\mathcal{F}) \subseteq \cl(\mathcal{F}')$ if $\mathcal{F} \subseteq \mathcal{F}'$ and $\cl(\cl(\mathcal{F})) = \cl(\mathcal{F})$ for all graph classes $\mathcal{F}$ and $\mathcal{F}'$.
% lax end
% lax end
	Furthermore, the following inclusions hold (and cannot be reversed, cf.\@ \cref{ex1}):
	
% lax begin Lax871432.IntersectionsUnions
% lax begin Lax871432Proofs.cl_iInf_le_iInf_cl
% lax begin Lax871432Proofs.iSup_cl_le_cl_iSup
...
% lax begin Lax871432.GraphProducts
	We write $G \times H$ for the \emph{categorical product} of $G$ and $H$, i.e.\@ $V(G \times H) \coloneqq V(G) \times V(H)$ and $gh$ and $g'h'$ are adjacent in $G \times H$ iff $gg' \in E(G)$ and $hh' \in E(H)$. The \emph{lexicographic product} $G \cdot H$ is defined as the graph with vertex set $V(G) \times V(H)$ and edges between $gh$ and $g'h'$ iff $g = g'$ and $hh' \in E(H)$ or $gg' \in E(G)$.
% lax end
	It is well-known, cf.\@ e.g.\@ \cite[(5.28)--(5.30)]{lovasz_large_2012}, that
	for all graphs $F_1, F_2, G_1, G_2$, and all connected graphs $K$, the following equalities hold:
	\begin{align}
% lax begin Lax871432.CoproductCounts
% lax begin Lax871432Proofs.homCount_sum_left
		\hom(F_1 + F_2, G) &= \hom(F_1, G) \hom(F_2, G), \label{eq:coproduct} \\
% lax end
% lax end
% lax begin Lax871432.CategoricalProductCounts
% lax begin Lax871432Proofs.homCount_catProd_right
		\hom(F, G_1 \times G_2) &= \hom(F, G_1) \hom(F, G_2), \text{ and }\label{eq:product} \\
% lax end
% lax end
% lax begin Lax871432.DisjointUnionCounts
% lax begin Lax871432Proofs.homCount_sum_right
...
% lax begin Lax871432.ConnectedPartitions
...
% lax end
% lax begin Lax871432.Contractions
...
% lax begin Lax871432.PreservationProperties
...
% lax begin Lax871432.IsomorphismRelaxations
...
% lax begin Lax871432.LinearCombinationLemma
...
% lax end
% lax begin Lax871432.ProductPreservation
...
% lax begin Lax871432.TakingSummands
...
\item $\cl(\mathcal{F})$ is minor-closed,\label{rcl4}
		\end{enumerate}
		the implications \ref{rcl1} $\Rightarrow$ \ref{rcl2} $\Leftrightarrow$ \ref{rcl4} hold.
	\end{theorem}
% lax end
...
% lax begin Lax871432.FullComplementCounts
	...
	\end{lemma}
% lax end
	\begin{proof}Write $S$ for the set of all maps $V(F) \to V(G)$. For $e \in E(F)$, write $A_e \subseteq S$ for set of all maps $h$ such that the image of $e$ under $h$ is an edge or a loop in $G$.
...
% lax begin Lax871432.LoopGraphs
...
% lax begin Lax871432.LoopedGraphCounts
	...
	\end{theorem}
% lax end
	\begin{proof}The proof is by establishing a bijection between the set of homomorphisms $h \colon F \to G^\circ$ and the set $X$ of pairs $(L, \phi)$ where $L \subseteq E(F)$ and $\phi \colon F \oslash L \to G$ is a homomorphism.	
...
\end{proof}
	...
	\end{lemma}
	\begin{proof}Let $F \in \mathcal{F}$ be a graph on $n$ vertices. Since the graph $nK_1$ can be obtained from $F$ by deleting all its edges, it holds that $nK_1 \in \mathcal{F}$.
...
% lax begin Lax871432.ComplementCounts
...
Since \cref{lem:lincomb} is not directly applicable since the condition in \ref{cl2} involves graphs with loops, we first prove the following claim:
		...
		\begin{claim} \label{cl:full-complement}
			...
			For all graphs $G$ and~$H$ with loops at every vertex, if $G \equiv_{\mathcal{F}} H$, then $G \equiv_{\cl(\mathcal{F})} H$.
		\end{claim}
		\begin{claimproof}
...
% lax begin Lax871432.InducedSubgraphs
...
% lax begin Lax871432.EdgeContractions
...
% lax begin Lax871432.LexicographicProductCounts
	...
	\end{theorem}
% lax end
	\begin{proof}The proof is by constructing a bijection between the set of homomorphisms $F \to G \cdot H$ and the set of triples $(\mathcal{R}, g, h)$ where $\mathcal{R} \in \Pi(V(F))$ is such that all $F[R]$, $R \in \mathcal{R}$, are connected, and $g \colon F/\mathcal{R} \to G$ and $h \colon \coprod_{R \in \mathcal{R}} F[R] \to H$ are homomorphisms.
	...
	\cref{thm:lexprod-hom} yields \cref{prop:lexprod-indsub,prop:lexprod-contract} as follows: 
	...
	\end{corollary}
	\begin{proof}Assuming \ref{ccontr1}, if $G \equiv_{\mathcal{F}} G'$ and $H \equiv_{\mathcal{F}} H'$ then $G \cdot H \equiv_{\mathcal{F}} G' \cdot H$ by \cref{prop:lexprod-contract}
	...
	\end{lemma}
	\begin{proof}Let $F \in \mathcal{F}$. Since every homomorphism distinguishing closed graph class is closed under disjoint unions, cf.\@ \cref{eq:coproduct} and \cite{roberson_oddomorphisms_2022}, it suffices to show that every connected component $C$ of $F$ is in $\mathcal{F}$. Let~$m$ denote the number of connected components of~$F$. 
	...
	\begin{proof}It is shown that $\equiv_{\mathcal{F}}$ is preserved under taking complements in the sense of \cref{thm:complement}.
	...
	\end{lemma}
	\begin{proof}The backward implication is immediate since $\mathcal{F}_K \subseteq \mathcal{F}$ and thus $\cl(\mathcal{F}_K) \subseteq \cl(\mathcal{F})$.
...
\end{proof}
	...
	\begin{example} \label{ex1} \label{ex:union}
		...
		\end{enumerate}
	\end{example}
	\begin{proof}For the first example, observe that clearly $\Gamma(\mathcal{F}) = \{F_1, F_2\}$.
	...
	\end{theorem}
	\begin{proof}Construct $\mathcal{F}'$ by choosing for every $\Lambda \subseteq \Gamma(\mathcal{F})$ a
	...
	\end{lemma}
	\begin{proof}Since the class of all graphs is not essentially profinite, there exists a number $t \in \mathbb{N}$ such that all $F \in \mathcal{F}$ do not contain the clique $K_t$ as a minor.
